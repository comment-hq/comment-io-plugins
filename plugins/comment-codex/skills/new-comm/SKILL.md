...
`ADOPTED`, then start the separate `listen bind` command with a 250 ms initial
yield. If bind returns a live command session, retain that session and the exact
mint data, then immediately continue with source resolution and writing in this
conversation. If it prints `ARMED` before yielding, record that success and
continue. Do not wait on a live bind while content work remains. An unavailable
...
Before handoff, reconcile any retained bind command session. If it is still
running, wait for it now; record `ARMED` on success or treat a failed bind as an
unavailable listener. Leave listening armed when available and report its honest
state.
