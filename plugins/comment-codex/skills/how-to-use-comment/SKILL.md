...
target context use `https://comt.dev`.
This plugin is pinned to `https://comt.dev` for the `staging` publication. Reject supplied Comm links and shortlinks whose resolved origin differs; do not use or pass another origin to the runtime.

Use the staging Comment.io tools already in this session (the host may prefix
